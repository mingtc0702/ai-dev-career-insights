...
export default function InteractiveDataExplorer() {
  ...
  useEffect(() => {
    const filtered = data.filter((row) => {
      ...
      const salary = row.normalized_salary || "";
      ...
      return (
        ...
        (filters.normalized_salary === "" ||
          salary.toString().includes(filters.normalized_salary)) &&
        (filters.salary_rank === "" ||
  ...
  return (
    <Paper sx={{ p: 3 }}>
      ...
      <Stack direction="row" spacing={2} flexWrap="wrap" mb={2}>
        ...
        {["title", "company_name", "normalized_salary", "salary_rank"].map(
          (col) => (
            <TextField
              key={col}
