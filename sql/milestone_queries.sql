-- 1.Top 10 highest-paid unique job titles
SELECT DISTINCT
   title,
   company_name,
   normalized_salary
...
-- 2.Count of Job Postings by Experience Level
SELECT
...
-- 3.Count how many respondents in the survey report knowing Python
SELECT
...
--4.Average Salary by U.S. State (Normalized Jobs Only)
SELECT
...
--5.Compare AI-Related Job Salaries vs Non-AI Jobs
SELECT
...
--6.Highest Paying Companies for AI Skills (only companies with ≥5 AI postings)
SELECT
   company_name,
...
--7.Join Survey Data and LinkedIn to Compare Skill Popularity
SELECT
   lang.language,
...
--8.Rank Tech Job Postings by Salary Within Job Family
SELECT DISTINCT
   job_family,
   title,
   company_name,
...
--9.Find Job Postings Matching Survey Respondents’ Experience Levels
...
-- 10.Linking Developer AI Sentiment with Labor-Market Demand for AI-Skilled Roles
